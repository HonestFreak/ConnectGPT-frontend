...
const adddoc = () => {
  ...
  const url_list = url.split('\n');
    try {
      const response = await fetch(`api/chat/urlmethod/?bot_id=${userInfo["bots"][currentbot]['id']}`
      , {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${accessToken}`,
  ...
  const textmethod = async () => {
    setloading(true)
      try {
        const response = await fetch(`api/chat/textmethod/?bot_id=${userInfo["bots"][currentbot]['id']}
        &text=${text}`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${accessToken}`,
