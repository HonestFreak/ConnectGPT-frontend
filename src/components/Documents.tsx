...
const Docs = () => {
  ...
  const fetchCols = async () => {
    try {
      const response = await fetch(`api.azurewebsites.net/collections/getcol/?bot_id=${userInfo["bots"][currentBot]['id']}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${accessToken}`,
          'Accept': 'application/json',
                  ...
                  <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                    <p className="text-sm font-medium text-success">
                      <input
                        type="submit"
                        onClick={() =>
                          fetch(`api.azurewebsites.net/collections/delindex/${docIndex[index]}`)
                        }
